...
class Quicksort:
    def sort(self, array: List[T], start: int = 0, end: Union[int, None] = None) -> List[T]:
        ...
        pivot_index = self._get_pivot_index(array, start, end)

        self._swap(array, start, pivot_index)

        pivot_index = start

        for i in range(start, end):
            if array[i] <= array[start]:
                self._swap(array, i, pivot_index)
                pivot_index += 1
    ...
    def _swap(self, array: List[T], l: int, r: int) -> None:
        if l != r:
            array[l], array[r] = array[r], array[l]

    def _get_pivot_index(self, array: List[T], start: int, end: int) -> int:
        size = end - start
